...
window.addEventListener('load', () => {
    ...
    async function loadData() {
        try {
            console.log('loadData() called - starting to load data...');
            ...
            // Always load fresh data from server, ignore localStorage cache
            console.log('Loading fresh cards from server...');
            const cardsResponse = await fetch('/api/load-cards');
            console.log('Server response status:', cardsResponse.status);
            if (cardsResponse.ok) {
            ...
            console.log('Global cards variable set to:', cards ? cards.length : 'undefined/null');
            
            // Debug: Check for Charger4 cards in the loaded data
            const charger4Cards = cards.filter(card => {
                const model = card.model || card.Model || '';
                const generation = card.generation || card.Generation || '';
                return model.toLowerCase().includes('charger') || generation.toLowerCase().includes('charger4');
            });
            console.log('Charger4 cards in loaded data:', charger4Cards.length);
        } catch (error) {
    ...
    function updateStatistics() {
        ...
        const totalHypaEl = document.getElementById('totalHypa');
        ...
        if (totalHypaEl) totalHypaEl.textContent = totalHypa;
...
Only cards for existing products will be imported.`;
    ...
    function extractFeatureCards(row, columns) {
        ...
        // Fallback: parse HTML block in shared.feature-1-card if no standard features found
        if (!foundStandard && row['shared.feature-1-card'] && row['shared.feature-1-card'].trim()) {
            try {
                const html = row['shared.feature-1-card'];
                const parser = new DOMParser();
                const doc = parser.parseFromString(html, 'text/html');
                // Find all card containers (divs with a header and body)
                ...
                console.error('Error parsing HTML block for feature cards:', err);
            }
        }
        return cards;
    }
    ...
    function extractOptionCards(row, columns) {
        ...
        // Fallback: scan all columns for HTML blocks representing option cards
        columns.forEach(col => {
            const cell = row[col];
            if (cell && cell.includes('<div class="card"')) {
                try {
                    const parser = new DOMParser();
                    const doc = parser.parseFromString(cell, 'text/html');
                    const cardDivs = Array.from(doc.querySelectorAll('div.card'));
                    cardDivs.forEach((div, idx) => {
                        const title = div.querySelector('.card-title')?.textContent?.trim() || '';
                        const description = div.querySelector('.card-description')?.textContent?.trim() || '';
                        const price = div.querySelector('.card-price')?.textContent?.trim() || '';
                        const img = div.querySelector('img');
                        const imageUrl = img ? img.getAttribute('src') : '';
                        if (title || description || imageUrl || price) {
                            cards.push({
                    ...
                    console.error('Error parsing HTML block for option cards:', err);
                }
            }
        });
        return cards;
    ...
    function extractCargoCards(row, columns) {
        ...
        // Fallback: scan all columns for HTML blocks representing cargo cards
        columns.forEach(col => {
            const cell = row[col];
            if (cell && cell.includes('<div class="card"')) {
                try {
                    ...
                    console.error('Error parsing HTML block for cargo cards:', err);
                }
            }
        });
        return cards;
    ...
    function extractWeatherCards(row, columns) {
        ...
        // Fallback: scan all columns for HTML blocks representing weather cards
        columns.forEach(col => {
            const cell = row[col];
            if (cell && cell.includes('<div class="card"')) {
                try {
                    ...
                    console.error('Error parsing HTML block for weather cards:', err);
                }
            }
        });
        return cards;
    ...
    // Function to populate export modal statistics
    function populateExportModalStatistics() {
        console.log('Populating export modal statistics...');
        
        // Get all cards from global variable or localStorage as fallback
        let allCards = cards || [];
        console.log('Global cards variable:', cards ? cards.length : 'undefined/null');
        ...
        // Debug: Check for Charger4 cards specifically
        ...
        const cardsWithImages = allCards.filter(card => card.imageUrl && card.imageUrl.trim()).length;
        const readyToExport = allCards.filter(card => 
            card.title && card.title.trim() && 
            card.content && card.content.trim()
        ).length;
        
        // Check for imported Hypa data from multiple sources
        let hypaData = window.originalHypaData || [];
        let hypaSource = 'window.originalHypaData';
        
        // If no data in window, try to load from cache file
        if (hypaData.length === 0) {
            try {
                // Try to fetch the cached Hypa data
                fetch('./data/hypaCsvCache.json')
                    .then(response => response.json())
                    .then(cacheData => {
                        if (cacheData && cacheData.data && cacheData.data.length > 0) {
                            console.log(`Loaded ${cacheData.data.length} metafields from cache file`);
                            hypaData = cacheData.data;
                            hypaSource = 'cache file';
                            
                            // Update the display with the loaded data
                            updateHypaStatistics(hypaData, hypaSource);
                        }
                    })
                    .catch(error => {
                        console.log('No cache file found or error loading cache:', error.message);
                    });
            } catch (error) {
                console.log('Error accessing cache file:', error.message);
            }
        }
        
        // Update the export modal elements
        const localTotalCardsEl = document.getElementById('localTotalCards');
        const localWithImagesEl = document.getElementById('localWithImages');
        const localReadyToExportEl = document.getElementById('localReadyToExport');
        
        if (localTotalCardsEl) localTotalCardsEl.textContent = totalCards;
        if (localWithImagesEl) localWithImagesEl.textContent = cardsWithImages;
        if (localReadyToExportEl) localReadyToExportEl.textContent = readyToExport;
        
        // Update Hypa statistics
        updateHypaStatistics(hypaData, hypaSource);
        
        console.log('Export modal statistics populated:', {
            totalCards,
            cardsWithImages,
            readyToExport,
            hypaDataLength: hypaData.length,
            hypaSource
        });
    }
    
    // Helper function to update Hypa statistics
    function updateHypaStatistics(hypaData, source) {
        const hypaExistingCount = hypaData.length;
        const hypaLastUpdated = hypaData.length > 0 ? `Imported (${source})` : 'Unknown';
        const hypaConnectionStatus = hypaData.length > 0 ? `${hypaExistingCount} metafields loaded` : 'No CSV imported';
        
        const hypaExistingCountEl = document.getElementById('hypaExistingCount');
        const hypaLastUpdatedEl = document.getElementById('hypaLastUpdated');
        const hypaConnectionStatusEl = document.getElementById('hypaConnectionStatus');
        
        if (hypaExistingCountEl) hypaExistingCountEl.textContent = hypaExistingCount;
        if (hypaLastUpdatedEl) hypaLastUpdatedEl.textContent = hypaLastUpdated;
        if (hypaConnectionStatusEl) hypaConnectionStatusEl.textContent = hypaConnectionStatus;
        
        console.log('Hypa statistics updated:', {
            hypaExistingCount,
            hypaLastUpdated,
            hypaConnectionStatus
        });
    }

    // Function to perform export comparison
    async function performExportComparison() {
        console.log('Performing export comparison...');
        
        // Get local cards from global variable or localStorage as fallback
        let localCards = cards || [];
        if (localCards.length === 0) {
            try {
                localCards = JSON.parse(localStorage.getItem('cards') || '[]');
            } catch (error) {
                console.error('Error parsing localStorage cards:', error);
                localCards = [];
            }
        }
        console.log(`Comparing ${localCards.length} local cards`);
        
        // Get Hypa data
        let hypaData = window.originalHypaData || [];
        if (hypaData.length === 0) {
            try {
                const response = await fetch('./data/hypaCsvCache.json');
                const cacheData = await response.json();
                if (cacheData && cacheData.data) {
                    hypaData = cacheData.data;
                    console.log(`Loaded ${hypaData.length} metafields from cache for comparison`);
                }
            } catch (error) {
                console.log('No cache file available for comparison');
            }
        }
        ...
        // Group local cards by SKU (using same logic as export manager)
        const localCardsBySku = groupLocalCardsBySku(localCards);
        console.log(`Grouped local cards into ${Object.keys(localCardsBySku).length} SKUs`);
        
        // Compare each SKU group with Hypa data
        Object.keys(localCardsBySku).forEach(sku => {
            const localSkuGroup = localCardsBySku[sku];
            const hypaSkuData = hypaData.find(row => row.sku === sku);
            
            // Compare each card in the SKU group
            localSkuGroup.cards.forEach(card => {
                const cardType = card.cardType || 'unknown';
                const cardPosition = card.position || 1;
                const hypaFieldName = `shared.${cardType}-${cardPosition}-card`;
                
                if (hypaSkuData) {
                    const hypaContent = hypaSkuData[hypaFieldName] || '';
                    const cardContent = card.content || '';
                    
                    if (!hypaContent.trim()) {
                        // New card - doesn't exist in Hypa
                        comparisonResults.new.push({
                            sku,
                            card,
                            action: 'new',
                            reason: 'Card does not exist in Hypa',
                            skuGroup: localSkuGroup
                        });
                    } else if (hypaContent.trim() !== cardContent.trim()) {
                        // Updated card - content is different
                        comparisonResults.updated.push({
                            sku,
                            card,
                            action: 'update',
                            reason: 'Content differs from Hypa version',
                            hypaContent: hypaContent,
                            skuGroup: localSkuGroup
                        });
                    } else {
                        // Keep card - identical content
                        comparisonResults.keep.push({
                            sku,
                            card,
                            action: 'keep',
                    ...
                    // New card - SKU doesn't exist in Hypa
                    comparisonResults.new.push({
                        sku,
                        card,
                        action: 'new',
                        reason: 'SKU does not exist in Hypa',
                        skuGroup: localSkuGroup
                    });
                }
            });
        ...
        // Store results globally for selection step
        globalComparisonResults = comparisonResults;
        window.lastComparisonResults = comparisonResults;
        
        // Update the comparison display
        updateComparisonDisplay(comparisonResults);
        
        // Show the comparison step
        showExportStep(2);
        
        console.log('Export comparison completed:', {
            new: comparisonResults.new.length,
            updated: comparisonResults.updated.length,
            keep: comparisonResults.keep.length,
            remove: comparisonResults.remove.length,
            excluded: comparisonResults.excluded.length
        });
    }
    
    // Function to group local cards by SKU (same logic as export manager)
    function groupLocalCardsBySku(cards) {
        const groupedBySku = {};
        
        cards.forEach(card => {
            // Extract configuration data
            const config = card.configuration || {};
            const brand = config.brand || card.brand;
            const model = config.model || card.model;
            const generation = config.generation || card.generation;
            
            // Get all SKUs for this card (primary SKU + variant SKUs)
            const cardSkus = [card.sku || card.productSku || 'unknown'];
    ...
    // Global variable to store comparison results for selection
    let globalComparisonResults = null;
    
    // Function to show export selection interface
    function showExportSelection() {
        console.log('Showing export selection interface...');
        
        // Store comparison results globally for selection
        globalComparisonResults = window.lastComparisonResults || {
            new: [],
            updated: [],
            keep: [],
            remove: [],
        ...
        console.log('Global comparison results:', globalComparisonResults);
        console.log('Window last comparison results:', window.lastComparisonResults);
        
        // Build the selection interface
        buildExportSelectionInterface();
        
        // Show the selection step
        showExportStep(3);
    }
    
    // Function to build the export selection interface
    function buildExportSelectionInterface() {
        console.log('Building export selection interface...');
        ...
        // Group comparison results by SKU
        const skuGroups = groupComparisonResultsBySku(globalComparisonResults);
        console.log('SKU groups:', skuGroups);
        
        // Sort SKUs alphabetically
        const sortedSkus = Object.keys(skuGroups).sort();
        console.log('Sorted SKUs:', sortedSkus);
        
        sortedSkus.forEach((sku, index) => {
            const skuGroup = skuGroups[sku];
            const skuCard = document.createElement('div');
            skuCard.className = 'sku-card product-card card mb-2';
            skuCard.dataset.skuIndex = index;
            skuCard.dataset.sku = sku;
            skuCard.dataset.model = skuGroup.model || '';
            skuCard.dataset.brand = skuGroup.brand || '';
            skuCard.dataset.generation = skuGroup.generation || '';
            
            // Calculate summary for this SKU
            const summary = {
                new: skuGroup.cards.filter(c => c.action === 'new').length,
                update: skuGroup.cards.filter(c => c.action === 'update').length,
                keep: skuGroup.cards.filter(c => c.action === 'keep').length,
                remove: skuGroup.cards.filter(c => c.action === 'remove').length
            };
            
            const totalCards = skuGroup.cards.length;
            const exportableCards = summary.new + summary.update;
            
            // Create compact card layout with SKU-level checkbox
            skuCard.innerHTML = `
                        ...
                        <div class="d-flex align-items-center">
                            <div class="form-check me-3">
                                <input class="form-check-input sku-checkbox" type="checkbox" 
                                       id="sku-${index}" 
                                       data-sku="${sku}"
                                       ${exportableCards > 0 ? '' : 'disabled'}
                                       ${exportableCards > 0 ? 'checked' : ''}>
                                <label class="form-check-label fw-bold" for="sku-${index}">
                                    ${skuGroup.brand || 'Unknown'} ${skuGroup.model || ''} ${skuGroup.generation || ''}
                                    <span class="text-primary ms-2">${sku}</span>
                                    <span class="text-muted ms-2">(${totalCards} cards)</span>
                                </label>
                            </div>
                        </div>
                        <div class="d-flex align-items-center">
                            <span class="badge bg-success me-1">${summary.new}</span>
                            <span class="badge bg-warning me-1">${summary.update}</span>
                            <span class="badge bg-info me-1">${summary.keep}</span>
                            <span class="badge bg-danger me-1">${summary.remove}</span>
                        </div>
                    </div>
                </div>
                <div class="card-body py-2">
                    <div class="row g-1">
                        ${renderCompactCardGrid(skuGroup.cards, sku, index)}
                    </div>
                </div>
            `;
    ...
    // Function to group comparison results by SKU
    function groupComparisonResultsBySku(results) {
        const skuGroups = {};
        ...
        allResults.forEach(result => {
            ...
            if (!skuGroups[sku]) {
                skuGroups[sku] = {
                    sku: sku,
                    brand: result.skuGroup?.brand,
                    model: result.skuGroup?.model,
                    generation: result.skuGroup?.generation,
                    cards: []
                };
            }
            skuGroups[sku].cards.push(result);
        });
        
        return skuGroups;
    }
    
    // Function to get action color for card headers
    function getActionColor(action) {
        switch (action) {
            case 'new': return '#28a745'; // Bootstrap success green
            case 'update': return '#ffc107'; // Bootstrap warning yellow
            case 'keep': return '#17a2b8'; // Bootstrap info blue
            case 'remove': return '#dc3545'; // Bootstrap danger red
            default: return '#6c757d'; // Bootstrap secondary gray
        }
    }
    
    // Utility to get correct image src for cards (same as card-manager.js)
    ...
    // Helper function to get public WebDAV URL
    function getPublicWebDavUrl(webdavPath) {
        ...
        const encodedPath = publicPath
            .replace(/&/g, '%26')
            .replace(/ /g, '%20');
        return webdavBaseUrl + encodedPath;
    }
    
    // Function to render compact card grid for selection
    function renderCompactCardGrid(cards, sku, skuIndex) {
        return cards.map((result, cardIndex) => {
            const card = result.card;
            const actionClass = result.action === 'new' ? 'success' : 
                              result.action === 'update' ? 'warning' : 
                              result.action === 'keep' ? 'info' : 'danger';
            
            // Use the same image handling system as the card manager
            ...
            return `
                <div class="col-md-4 col-lg-3 col-xl-2">
                    <div class="card h-100 border-0 shadow-sm" style="min-height: 140px;">
                        <div class="card-header p-1 text-center" style="background-color: ${getActionColor(result.action)}; min-height: 20px;">
                            <small class="text-white fw-bold">${getCardTypeDisplayName(card.cardType).replace('-', ' ').toUpperCase()}</small>
                        </div>
                        <div class="card-body p-2 text-center">
                            ${hasImage ? `
                                <div class="mb-2">
                                    <img src="${imageSrc}" alt="${card.title || 'Card'}" 
                                         class="img-thumbnail" style="width: 60px; height: 60px; object-fit: cover;"
                                         onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
                                    <div class="bg-light rounded d-flex align-items-center justify-content-center" 
                                         style="width: 60px; height: 60px; margin: 0 auto; display: none;">
                                        <i class="fas fa-image text-muted" style="font-size: 1rem;"></i>
                                    </div>
                                </div>
                            ` : `
                                <div class="mb-2">
                            ...
                            `}
                            <div class="mb-1">
                                <small class="text-muted fw-bold">${card.title || 'Untitled'}</small>
                            </div>
                            <span class="badge bg-${actionClass}" style="font-size: 0.6rem;">
    ...
    // Function to setup selection event listeners
    function setupSelectionEventListeners() {
        // SKU-level checkboxes
        document.querySelectorAll('.sku-checkbox').forEach(checkbox => {
            checkbox.addEventListener('change', function() {
                const sku = this.dataset.sku;
                const isChecked = this.checked;
                
                // Update all card checkboxes for this SKU
                document.querySelectorAll(`.card-checkbox[data-sku="${sku}"]`).forEach(cardCheckbox => {
                    if (!cardCheckbox.disabled) {
        ...
        document.querySelectorAll('.card-checkbox').forEach(checkbox => {
            checkbox.addEventListener('change', function() {
                updateExportSelectionSummary();
                updateSkuCheckboxState(this.dataset.sku);
            });
        });
    ...
    function applySearchFilter() {
        ...
        console.log('Applying search filter:', { searchTerm, showFilter });
        ...
        productCards.forEach(card => {
            ...
            const matchesSearch = !searchTerm || 
                ...
                generation.toLowerCase().includes(searchTerm);
            
            // Check if card matches action filter
            let matchesActionFilter = true;
            if (showFilter !== 'all') {
                const cardCheckboxes = card.querySelectorAll('.card-checkbox');
                const hasMatchingAction = Array.from(cardCheckboxes).some(checkbox => {
                    const action = checkbox.dataset.action;
                    return showFilter === action;
                });
                matchesActionFilter = hasMatchingAction;
            }
            
            // Show/hide card based on filters
            if (matchesSearch && matchesActionFilter) {
        ...
        // Show/hide "Select Filtered Results" button based on whether we have filtered results
        const selectFilteredResultsBtn = document.getElementById('selectFilteredResultsBtn');
        if (selectFilteredResultsBtn) {
            if (searchTerm || showFilter !== 'all') {
                selectFilteredResultsBtn.style.display = 'inline-block';
            } else {
                selectFilteredResultsBtn.style.display = 'none';
            }
        }
        
        console.log('Search filter applied, visible cards:', visibleCount);
    }
    
    // Function to select all currently visible cards
    // Simple function to select all SKUs
    function selectAllSKUs() {
        ...
        // Select all individual card checkboxes
        ...
        console.log('Selected all SKUs');
    }
    
    // Simple function to deselect all SKUs
    function deselectAllSKUs() {
        console.log('deselectAllSKUs called');
        const skuCheckboxes = document.querySelectorAll('.sku-checkbox');
        console.log('Found SKU checkboxes:', skuCheckboxes.length);
        
        skuCheckboxes.forEach(checkbox => {
            checkbox.checked = false;
        });
        
        // Deselect all individual card checkboxes
        const cardCheckboxes = document.querySelectorAll('.card-checkbox');
        console.log('Found card checkboxes:', cardCheckboxes.length);
        ...
        console.log('Deselected all SKUs');
    }
    ...
    function toggleSelectAll() {
        ...
        const skuCheckboxes = document.querySelectorAll('.sku-checkbox');
        const anySelected = Array.from(skuCheckboxes).some(checkbox => checkbox.checked);
        
        console.log('Any SKUs selected:', anySelected);
        console.log('SKU checkboxes found:', skuCheckboxes.length);
        
        if (anySelected) {
            console.log('Deselecting all SKUs...');
            deselectAllSKUs();
        } else {
            console.log('Selecting all SKUs...');
            selectAllSKUs();
        }
    }
    
    // Simple clear function (same as deselect all)
    function clearAllSelections() {
        deselectAllSKUs();
    }
    
    // Function to clear search box only
    function clearSearchBox() {
        const searchInput = document.getElementById('productSearchFilter');
        if (searchInput) {
            searchInput.value = '';
            // Trigger search to refresh the display
            applySearchFilter();
        }
        console.log('Search box cleared');
    }
    
    // Function to select filtered results
    function selectFilteredResults() {
        console.log('selectFilteredResults called');
        
        // Get all visible product cards (not hidden by search/filter)
        const allProductCards = document.querySelectorAll('.product-card');
        console.log('Total product cards found:', allProductCards.length);
        
        let selectedCount = 0;
        
        allProductCards.forEach(productCard => {
            // Check if this product card is visible (not explicitly hidden)
            const isHidden = productCard.style.display === 'none';
            console.log('Product card hidden:', isHidden);
            
            if (!isHidden) {
                // Get the SKU checkbox for this product
                const skuCheckbox = productCard.querySelector('.sku-checkbox');
                if (skuCheckbox && !skuCheckbox.disabled) {
                    skuCheckbox.checked = true;
                    selectedCount++;
                    console.log('Selected SKU:', skuCheckbox.dataset.sku);
                }
            }
        });
        
        updateExportSelectionSummary();
        console.log('Selected filtered results:', selectedCount);
    }
    
    // Make functions globally accessible
    window.toggleSelectAll = toggleSelectAll;
    window.clearAllSelections = clearAllSelections;
    window.selectAllSKUs = selectAllSKUs;
    window.deselectAllSKUs = deselectAllSKUs;
    window.clearSearchBox = clearSearchBox;
    window.selectFilteredResults = selectFilteredResults;
    
    // Function to update SKU checkbox state based on card selections
    function updateSkuCheckboxState(sku) {
        const skuCheckbox = document.querySelector(`.sku-checkbox[data-sku="${sku}"]`);
        if (!skuCheckbox) return;
        
        const cardCheckboxes = document.querySelectorAll(`.card-checkbox[data-sku="${sku}"]:not(:disabled)`);
        const checkedCards = document.querySelectorAll(`.card-checkbox[data-sku="${sku}"]:not(:disabled):checked`);
        
        if (checkedCards.length === 0) {
            skuCheckbox.checked = false;
            skuCheckbox.indeterminate = false;
        } else if (checkedCards.length === cardCheckboxes.length) {
            skuCheckbox.checked = true;
            skuCheckbox.indeterminate = false;
        } else {
            skuCheckbox.checked = false;
    ...
    function updateToggleButtonState() {
        ...
        if (anySelected) {
            toggleBtn.innerHTML = '<i class="fas fa-times me-1"></i>Select None';
            toggleBtn.className = 'btn btn-sm btn-outline-danger';
        } else {
            toggleBtn.innerHTML = '<i class="fas fa-check-double me-1"></i>Select All';
            toggleBtn.className = 'btn btn-sm btn-outline-primary';
        }
    }
    
    // Function to update export selection summary
    function updateExportSelectionSummary() {
        const selectedCheckboxes = document.querySelectorAll('.card-checkbox:checked:not(:disabled)');
        ...
        if (summaryNew) summaryNew.textContent = summary.new;
        if (summaryUpdated) summaryUpdated.textContent = summary.update;
        if (summaryRemove) summaryRemove.textContent = summary.remove;
        if (summaryTotal) summaryTotal.textContent = summary.total;
        
        // Enable/disable continue button
        if (continueToExportBtn) {
            continueToExportBtn.disabled = summary.total === 0;
        }
        
        console.log('Export selection summary updated:', summary);
    }
    
    // Function to update comparison display
    function updateComparisonDisplay(results) {
        console.log('updateComparisonDisplay called with results:', {
        ...
        const newCountEl = document.getElementById('exportNewCount');
        const updateCountEl = document.getElementById('exportUpdateCount');
        const skipCountEl = document.getElementById('exportSkipCount');
        const conflictCountEl = document.getElementById('exportConflictCount');
        const excludedCountEl = document.getElementById('exportExcludedCount');
        
        console.log('Summary count elements found:', {
            newCountEl: !!newCountEl,
            updateCountEl: !!updateCountEl,
            skipCountEl: !!skipCountEl,
            conflictCountEl: !!conflictCountEl,
            excludedCountEl: !!excludedCountEl
        });
        
        if (newCountEl) newCountEl.textContent = results.new.length;
        if (updateCountEl) updateCountEl.textContent = results.updated.length;
        if (skipCountEl) skipCountEl.textContent = results.keep.length;
        if (conflictCountEl) conflictCountEl.textContent = results.remove.length;
        if (excludedCountEl) excludedCountEl.textContent = results.excluded.length;
        ...
        if (tableBody) {
            ...
            const allResults = [
                ...results.new.map(r => ({...r, actionClass: 'success'})),
                ...results.updated.map(r => ({...r, actionClass: 'warning'})),
                ...results.keep.map(r => ({...r, actionClass: 'info'})),
                ...results.remove.map(r => ({...r, actionClass: 'danger'}))
            ];
            
            allResults.forEach(result => {
                const row = document.createElement('tr');
                row.className = `table-${result.actionClass}`;
                
                // Get SKU group info
                const skuGroup = result.skuGroup || {};
                const skuInfo = skuGroup.brand || skuGroup.model || skuGroup.generation ? 
                    `${skuGroup.brand || ''} ${skuGroup.model || ''} ${skuGroup.generation || ''}`.trim() : '';
                
                row.innerHTML = `
                    <td>
                        <strong>${result.sku}</strong>
                        ${skuInfo ? `<br><small class="text-muted">${skuInfo}</small>` : ''}
                    </td>
                    <td>${getCardTypeDisplayName(result.card.cardType)} ${result.card.position || 1}</td>
                    <td>
                        <span class="badge bg-primary">Local</span>
                        ${result.card.title ? `<br><small>${result.card.title}</small>` : ''}
                    </td>
                    <td>
                        <span class="badge bg-secondary">Hypa</span>
                        ${result.hypaContent ? `<br><small>${result.hypaContent.substring(0, 50)}...</small>` : '<small>Not found</small>'}
                    </td>
                    <td>
                        <span class="badge bg-${result.actionClass}">${result.action.toUpperCase()}</span>
                    </td>
                    <td><small>${result.reason}</small></td>
                `;
    ...
    // Function to show export step
    function showExportStep(step) {
        console.log(`Showing export step ${step}...`);
        ...
        if (comparisonStep) comparisonStep.style.display = 'none';
        if (selectionStep) selectionStep.style.display = 'none';
        
        console.log('Steps hidden');
        ...
        switch (step) {
            case 1:
                if (analysisStep) {
                    ...
                    console.log('Analysis step displayed');
                } else {
                    console.error('Analysis step element not found!');
                }
                break;
            case 2:
                if (comparisonStep) {
                    comparisonStep.style.display = 'block';
                    console.log('Comparison step displayed');
                } else {
                    console.error('Comparison step element not found!');
                }
                break;
            case 3:
                if (selectionStep) {
                    ...
                    console.error('Selection step element not found!');
                }
                break;
        }
        ...
        const step1Badge = document.getElementById('exportStep1Badge');
        const step2Badge = document.getElementById('exportStep2Badge');
        const step3Badge = document.getElementById('exportStep3Badge');
        const step4Badge = document.getElementById('exportStep4Badge');
        
        if (progressBar) progressBar.style.width = `${step * 25}%`;
        
        if (step1Badge) step1Badge.className = step >= 1 ? 'badge bg-warning' : 'badge bg-secondary';
        if (step2Badge) step2Badge.className = step >= 2 ? 'badge bg-warning' : 'badge bg-secondary';
        if (step3Badge) step3Badge.className = step >= 3 ? 'badge bg-warning' : 'badge bg-secondary';
        if (step4Badge) step4Badge.className = step >= 4 ? 'badge bg-warning' : 'badge bg-secondary';
        
        // Update modal footer based on current step
        updateModalFooter(step);
        
        console.log(`Export step ${step} displayed`);
    }
    
    // Function to update modal footer based on current step
    function updateModalFooter(step) {
        const modalFooter = document.querySelector('#exportHypaModal .modal-footer');
        ...
        switch (step) {
            case 1: // Analysis step
                footerHtml = `
                    <div class="d-flex justify-content-between align-items-center w-100">
                        ...
                        <div class="d-flex gap-2">
                            <button type="button" class="btn btn-sm btn-primary" id="startAnalysisBtn">
                                <i class="fas fa-arrow-right me-1"></i>Continue to Comparison
                            </button>
                ...
                `;
                break;
            case 2: // Comparison step
                footerHtml = `
                    <div class="d-flex justify-content-between align-items-center w-100">
                        <button type="button" class="btn btn-sm btn-secondary" id="backToAnalysisBtn">
                            <i class="fas fa-arrow-left me-1"></i>Back to Analysis
                        </button>
                        <div class="d-flex gap-2">
                            <button type="button" class="btn btn-sm btn-primary" id="continueToSelectionBtn">
                                <i class="fas fa-arrow-right me-1"></i>Continue to Selection
                            </button>
                            <button type="button" class="btn btn-sm btn-secondary" data-bs-dismiss="modal">
                                Cancel
                            </button>
                ...
                `;
                break;
            case 3: // Selection step
                footerHtml = `
                    <div class="d-flex justify-content-between align-items-center w-100">
                        <button type="button" class="btn btn-sm btn-secondary" id="backToComparisonBtn">
                            <i class="fas fa-arrow-left me-1"></i>Back to Comparison
                        </button>
                        <div class="d-flex gap-2">
                            <button type="button" class="btn btn-sm btn-primary" id="continueToExportBtn" disabled>
                                <i class="fas fa-arrow-right me-1"></i>Continue to Export
                            </button>
                            <button type="button" class="btn btn-sm btn-secondary" data-bs-dismiss="modal">
                                Cancel
                            </button>
        ...
        attachExportNavigationListeners();
    }

    // Add event listeners for export modal
    function setupExportModalListeners() {
        console.log('Setting up export modal listeners...');
        
        // Listen for export modal opening
        const exportModal = document.getElementById('exportHypaModal');
        console.log('Export modal element:', exportModal);
        
        if (exportModal) {
            exportModal.addEventListener('shown.bs.modal', async function() {
                console.log('Export modal opened, populating statistics...');
                
                // Ensure data is loaded before populating statistics
                if (!cards || cards.length === 0) {
                    console.log('No cards loaded, loading data first...');
                    await loadData();
                }
                
                populateExportModalStatistics();
                // Re-attach event listeners when modal opens
                attachExportNavigationListeners();
            });
            console.log('Export modal event listener attached');
        } else {
            console.log('Export modal element not found');
        }
        
        // Listen for start analysis button click
        const startAnalysisBtn = document.getElementById('startAnalysisBtn');
        console.log('Start analysis button element:', startAnalysisBtn);
        
        if (startAnalysisBtn) {
            startAnalysisBtn.addEventListener('click', function() {
                console.log('Start analysis button clicked...');
                performExportComparison();
            });
            console.log('Start analysis button event listener attached');
        } else {
            console.log('Start analysis button element not found');
        }
    }

    // Function to attach export navigation listeners
    function attachExportNavigationListeners() {
        console.log('Attaching export navigation listeners...');
        ...
        if (backToAnalysisBtn) {
            backToAnalysisBtn.onclick = function() {
                console.log('Back to analysis clicked');
                showExportStep(1);
            };
            console.log('Back to analysis button listener attached');
        }
        
        if (continueToSelectionBtn) {
            continueToSelectionBtn.onclick = function() {
                console.log('Continue to selection clicked...');
                showExportSelection();
            };
            console.log('Continue to selection button listener attached');
        }
        
        if (backToComparisonBtn) {
            backToComparisonBtn.onclick = function() {
                console.log('Back to comparison clicked');
                showExportStep(2);
            };
            console.log('Back to comparison button listener attached');
        }
        
        if (continueToExportBtn) {
            continueToExportBtn.onclick = function() {
                console.log('Continue to export clicked...');
                generateHypaCsv();
            };
            console.log('Continue to export button listener attached');
        }
    }

            // Call setup function immediately and also on DOMContentLoaded
        setupExportModalListeners();
        
        if (document.readyState === 'loading') {
            document.addEventListener('DOMContentLoaded', setupExportModalListeners);
        }
        
        // Add event listeners for comparison step navigation
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (continueToSelectionBtn) {
                continueToSelectionBtn.addEventListener('click', function() {
                    console.log('Continue to selection clicked...');
                    showExportSelection();
                });
            }
            ...
            if (continueToExportBtn) {
                continueToExportBtn.addEventListener('click', function() {
                    console.log('Continue to export clicked...');
                    // This will be implemented next - the actual export functionality
                    alert('Export functionality will be implemented next. This will generate the CSV file for import into Hypa.');
                });
            }
        });
}); 