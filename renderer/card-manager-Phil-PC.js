...
class CardManager {
    ...
    async loadCards() {
        try {
            ...
            // Migrate any 'option' type cards to 'product-options' for consistency
            cardsData = cardsData.map(card => {
                if (card.cardType === 'option' || card.type === 'option') {
                    console.log(`[loadCards] Migrating card ${card.id} from 'option' to 'product-options'`);
    ...
    applyFilters() {
        ...
        const cardTypeMap = {
            'feature': ['feature'],
            'product-options': ['product-options', 'option'],  // Handle both 'product-options' and 'option'
            'option': ['product-options', 'option'],  // Handle both 'product-options' and 'option'
...
<div class="se-component se-image-container __se__float- __se__float-none">
    ...
    generateCargoOptionsCardHtml(card, imageUrl) {
        ...
        // If no valid image URL, generate HTML without image section
        if (!imageUrl || imageUrl.trim() === '') {
            return `<!-- Cargo Options Card Container -->
<div style="display: flex; align-items: center; gap: 20px; max-width: 1200px; margin: 10px auto 0 auto; padding: 20px; border: 1px solid #ddd; border-radius: 10px; background: #f9f9f9; flex-wrap: wrap;">
...
<div style="flex: 0 0 auto; max-width: 400px; min-width: 250px;">
    ...
    async startUpload(cardsToUpload) {
        ...
        if (successCount > 0) {
            ...
            uploadResults.filter(r => r.success).forEach(result => {
                ...
                // Handle variants properly - they can be strings or objects with name/sku
                let variantsText = '';
                if (Array.isArray(config.variants)) {
                    variantsText = config.variants.map(variant => {
                        if (typeof variant === 'string') {
                            return variant;
                        } else if (typeof variant === 'object' && variant.name) {
                            return variant.name;
                    ...
                    variantsText = String(config.variants);
                }
                
                finalMessage += `<li><strong>${model} ${generation}</strong> (${variantsText}) - Model: ${model}, Generation: ${generation}, Variants: ${variantsText}</li>`;
            });
            finalMessage += '</ul>';
        }
    ...
    async uploadSingleImage(card) {
        ...
        const response = await fetch('/api/bulk-upload-image', {
            ...
            body: JSON.stringify({
                cardId: card.id,  // Backend expects 'cardId'
