...
function startServer() {
  ...
  // API route to proxy external images and serve local images for preview
  app.get('/api/image-proxy', async (req, res) => {
    ...
    try {
      // Check if this is a local image (starts with cards/images/)
      if (imageUrl.startsWith('cards/images/') || imageUrl.startsWith('/cards/images/')) {
        // Serve local image from the images directory
        const cleanPath = imageUrl.replace(/^\/+/, ''); // Remove leading slashes
        const imagePath = path.join(__dirname, 'renderer', cleanPath);
        ...
        if (!normalizedImagePath.startsWith(normalizedImagesDir)) {
          return res.status(403).json({ error: 'Access denied: File is outside the images directory' });
        }
        ...
        // Handle external URLs (existing functionality)
...
ipcMain.handle('save-image-to-disk', async (event, { imageData, extension }) => {
  try {
    ...
    // Return the relative path that can be used in cards
    const relativePath = `renderer/cards/images/${filename}`;
    console.log(`[save-image-to-disk] Image saved: ${relativePath}`);
    return relativePath;
  } catch (error) {
    console.error('[save-image-to-disk] Error saving image:', error);
    throw error;
  }
});
...
ipcMain.handle('cleanup-orphaned-images', async () => {
  try {
    ...
    if (!fsSync.existsSync(imagesDir)) {
      return { success: true, deleted: 0, message: 'Images directory does not exist' };
    }
    if (!fsSync.existsSync(cardsDir)) {
      return { success: true, deleted: 0, message: 'Cards directory does not exist' };
    }

    // Get all image files
    const imageFiles = await fs.readdir(imagesDir);
    const imageExtensions = ['.jpg', '.jpeg', '.png', '.gif', '.webp', '.bmp'];
    const imageFilesOnly = imageFiles.filter(file => 
      imageExtensions.some(ext => file.toLowerCase().endsWith(ext))
    );

    // Get all card files and extract referenced images
    const cardFiles = await fs.readdir(cardsDir);
    const jsonFiles = cardFiles.filter(file => file.endsWith('.json'));
    
    const referencedImages = new Set();
    
    for (const cardFile of jsonFiles) {
      try {
        const cardContent = await fs.readFile(path.join(cardsDir, cardFile), 'utf8');
        ...
        // Check thumbnail field if it exists
        if (card.thumbnail) {
          // If thumbnail is a base64 string, it's embedded, not a file
          if (!card.thumbnail.startsWith('data:')) {
            const thumbnailName = card.thumbnail.split('/').pop();
            if (thumbnailName) {
        ...
        console.warn(`[cleanup-orphaned-images] Error reading card file ${cardFile}:`, error.message);
      }
    }

    // Find orphaned images (not referenced in any card)
    const orphanedImages = imageFilesOnly.filter(imageFile => !referencedImages.has(imageFile));
    
    // Delete orphaned images
    let deletedCount = 0;
    for (const orphanedImage of orphanedImages) {
      try {
        const imagePath = path.join(imagesDir, orphanedImage);
        await fs.unlink(imagePath);
        deletedCount++;
        console.log(`[cleanup-orphaned-images] Deleted orphaned image: ${orphanedImage}`);
      } catch (error) {
        console.warn(`[cleanup-orphaned-images] Error deleting ${orphanedImage}:`, error.message);
      }
    }

    const message = `Cleanup completed. Deleted ${deletedCount} orphaned images out of ${imageFilesOnly.length} total images.`;
    console.log(`[cleanup-orphaned-images] ${message}`);
    ...
    console.error('[cleanup-orphaned-images] Error during cleanup:', error);
    throw error;
  }
});

ipcMain.handle('delete-image-file', async (event, imagePath) => {
  try {
    ...
    if (path.isAbsolute(imagePath)) {
      ...
      // Remove any leading slashes and ensure it's relative to renderer directory
      const cleanPath = imagePath.replace(/^\/+/, '');
      fullPath = path.join(__dirname, 'renderer', cleanPath);
    }

    // Verify the file exists and is within the images directory
    if (!fsSync.existsSync(fullPath)) {
      console.warn(`[delete-image-file] File does not exist: ${fullPath}`);
    ...
    if (!normalizedFullPath.startsWith(normalizedImagesDir)) {
      throw new Error('Access denied: File is outside the images directory');
    }

    await fs.unlink(fullPath);
    console.log(`[delete-image-file] Image deleted: ${fullPath}`);
    
    return { success: true, message: 'Image deleted successfully' };
  } catch (error) {
    console.error('[delete-image-file] Error deleting image:', error);
    throw error;
  }
}); 
