...
function processTemplate(template, cardData) {
  ...
  // Compute display price with currency symbol when appropriate
  const rawPrice = cardData.price || '';
  let displayPrice = '';
  if (rawPrice && String(rawPrice).trim() !== '') {
    const p = String(rawPrice).trim();
    if (/^(£|€|\$)/.test(p) || /^(tbc|no extra cost)$/i.test(p)) {
      displayPrice = p;
    } else {
      displayPrice = '£' + p;
    }
  ...
  const placeholders = {
    ...
    // Support templates that use {description} instead of {content}
    '{description}': cardData.description || cardData.content || '',
    '{imageUrl}': cardData.imageUrl || '',
    '{price}': cardData.price || '',
    '{priceWithSymbol}': displayPrice,
